...
export class PaymentsService implements IPaymentService {
    ...
    private calculateNextDueDate(current: Date, interval: 'daily' | 'weekly' | 'monthly'): Date {
  ...
  return date;
    ...
    async getPaymentsByUserId(userId: string, limit: number, offset: number): Promise<Payment[]> {
  ...
  const allowedStatuses: PaymentStatus[] = [PaymentStatus.PENDING, PaymentStatus.REJECTED];
  if (!allowedStatuses.includes(payment.status)) {
    throw new Error('Cannot upload proof for this payment status');
  }

  const proofUrl = await this.fileUploadService.uploadFile(data.file, 'payments');
...
async processRecurringPayments(): Promise<void> {
  const recurringPaymentsDue = await this.paymentRepository.findRecurringPaymentsDue();

  for (const payment of recurringPaymentsDue) {
    try {
      if (!payment.recurringPaymentId) continue;

      const recurringPayment = await this.paymentRepository.findRecurringPaymentById(payment.recurringPaymentId);
      ...
      const newPayment = await this.createPayment({
        ...
        description: `Recurring payment - ${payment.description}`,
      });
      ...
      await this.paymentRepository.updateRecurringPayment(recurringPayment.id, {
        nextDueDate,
      });

    } catch (error) {
      console.error(`Error processing recurring payment for user ${payment.userId}:`, error);
    }
  }
}


  async createRecurringPayment(paymentId: string, frequency: string): Promise<void> {
  }

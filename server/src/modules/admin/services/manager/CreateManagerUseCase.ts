...
export class CreateManagerUseCase {
    ...
    async execute(createManagerDto: CreateManagerDto, createdBy: string) {
        ...
        const existingUser = await this.managerRepository.findByEmail(createManagerDto.email);
        if (existingUser) {
            throw new ConflictException('Email already exists');
        }
        // Validate dormitory exists
        const dormitory = await this.dormitoryService.findOne(createManagerDto.dormitoryId);
        if (!dormitory) {
        ...
        // Create manager
        const manager = await this.managerRepository.create(createManagerDto, hashedPassword);

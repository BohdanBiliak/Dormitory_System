...
export class ManagerRepository implements IManagerRepository {
    ...
    async findAll(filters: ManagerFiltersDto): Promise<{ managers: UserWithRelations[]; total: number }> {
        const { page = 1, limit = 10, search, sortBy = 'displayName', show, dormitoryId } = filters;
    ...
    async assignToDormitory(managerId: string, dormitoryId: string): Promise<void> {
    ...
    async removeFromDormitory(managerId: string, dormitoryId: string): Promise<void> {

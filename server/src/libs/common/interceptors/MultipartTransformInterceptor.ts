  ...
  private parseFloorAssignments(body: any): any[] {
    ...
    Object.keys(body).forEach(key => {
      ...
      if (floorMatch) {
        ...
        const roomMatch = key.match(/^floorAssignments\[(\d+)\]\[roomAssignments\]\[(\d+)\]\[(\w+)\]$/);
        if (roomMatch) {
            ...
            if (typeof roomNumbers === 'string') {
              if (roomNumbers.startsWith('[') && roomNumbers.endsWith(']')) {
                try {
                  ...
                  roomNumbers = roomNumbers.slice(1, -1).split(',').map(n => parseInt(n.trim()));
                }
              } else if (roomNumbers.includes(',')) {
                roomNumbers = roomNumbers.split(',').map(n => parseInt(n.trim()));
              } else {

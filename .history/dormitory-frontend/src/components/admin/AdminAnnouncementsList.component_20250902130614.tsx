...
export function AdminAnnouncementList(){
    ...
    const handleChangeShowExpired = (): void => {
        setShowExpired(!showExpired);
    };
    
    const handleChangeShowHidden = (): void => {
    ...
    // Use useEffect to update announcements when data changes
    useEffect(() => {
        if (announcementsList && !isLoading) {
            setAnnouncements(announcementsList.data || []);
        }
    }, [announcementsList, isLoading, showHidden, showExpired, page]);

    console.log("showExpired ", showExpired);
    console.log("showHidden:", showHidden);
    console.log("announcements:", announcements);

    // Fix: use announcementsList instead of data
    const totalPages = announcementsList?.totalPages || 1;
                ...
                <table className="w-full table-fixed border-collapse">
                    ...
                    {announcements.map((announcement, index) => (
                        <tr key={index}>
                            ...
                            <td className="border border-black"><Link href={`/admin/announcements/${announcement.id}`} >{announcement.title}</Link></td>
